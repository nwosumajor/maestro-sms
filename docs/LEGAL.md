# MAESTRO-SMS — Platform Legal & Policy Documents

**Operator:** MajorGBN Innovations Limited ("MajorGBN", "we", "us") — RC No. `[●]`, registered office `[●]`, Nigeria.
**Platform:** MAESTRO-SMS, the multi-tenant School Management System at `[platform domain]` (the "Platform").

> **⚠️ DRAFT — LEGAL REVIEW REQUIRED.** These documents were prepared to accurately
> reflect how the Platform is engineered and operated, but they are **not legal
> advice** and must be reviewed, adapted and approved by qualified Nigerian counsel
> before publication or execution — in particular for compliance with the **Nigeria
> Data Protection Act 2023 (NDPA)**, the **NDPR 2019** and NDPC guidance. Because the
> Platform processes the personal data of **children at scale**, MajorGBN is likely a
> data controller/processor **of major importance** under the NDPA and must register
> with the **Nigeria Data Protection Commission (NDPC)** and may require a Data
> Protection Officer. Items marked `[●]` are business decisions or facts counsel/
> management must fill in.

**Version:** 1.0-draft · **Effective date:** `[●]` · Documents in this file:

1. [Privacy Policy](#1-privacy-policy)
2. [Data Processing Agreement (DPA)](#2-data-processing-agreement-dpa)
3. [Refund Policy](#3-refund-policy)
4. [Master Service Agreement (MSA)](#4-master-service-agreement-msa)
5. [Cyber Liability & Security Addendum](#5-cyber-liability--security-addendum)
6. [Implementation guide](#6-implementation-guide) (how to put these into force on the Platform)
...
## 5. Cyber Liability & Security Addendum
...
How to put these documents into force — recommended, in order:

### 6.1 Legalise before publishing
1. **Counsel review** (Nigerian, data-protection literate). Resolve every `[●]`: refund windows, caps, insurance, arbitration vs courts, retention numbers, hosting region.
2. **NDPC registration** as a data controller/processor of major importance (children's data at scale); appoint/designate a **DPO**; complete the NDPA-required **DPIA** for children's-data processing (the Privacy Policy §1.2–1.3 inventory is the starting point).
3. **Procure cyber liability insurance** before signing §5.2's commitment at scale.

### 6.2 Publish on the Platform (engineering work — I can build all of this)
1. **Public legal pages**: `/legal/privacy`, `/legal/terms` (MSA), `/legal/dpa`, `/legal/refunds`, `/legal/security` (Cyber Addendum), `/legal/subprocessors` — server-rendered from versioned markdown in the repo (single source of truth, reviewed via PRs like code), linked from the site footer, onboarding form, checkout, and in-app help.
2. **Clickwrap acceptance with an audit trail** — the legally load-bearing part:
   - *Onboarding:* the public onboarding form and the operator's "Approve & provision" flow require the School contact to tick acceptance of the MSA + DPA + Privacy Policy; store `{documentVersion, acceptedByUserId/email, IP, timestamp}` — the Platform's existing consent + audit-log machinery is exactly the right substrate.
   - *Checkout:* every `checkout/init` records the then-current MSA/Refund-Policy version in the payment's audit metadata (one line of code — the audit call is already there).
   - *First login of provisioned admins:* a one-time acceptance interstitial for accounts created by the operator (who never saw the onboarding form).
3. **Versioning & re-acceptance**: keep `version` + `effectiveDate` front-matter on each document; on a **material** change, an in-app banner (the AppShell renewal-banner pattern) requires billing-admin re-acceptance; non-material changes just notify. Never retro-apply a version nobody accepted — the audit trail proves which version governed which payment.
4. **Enterprise counter-signing**: for Enterprise deals, generate the DPA + Cyber Addendum as a PDF (the platform's pdfkit pipeline) pre-filled with the School's details for wet/e-signature, stored in the Document Vault.
5. **Operational hooks that make the documents true**: the subprocessor page must be updated *before* enabling a new provider; the DPA's deletion window needs a scheduled tenant-purge job wired to termination (the retention-job pattern); §5.4's 72-hour clock should be backed by an internal incident runbook and the operator-alert channel.

### 6.3 Keep them honest
Treat these documents like code: they live in this repo, changes go through review, and each release notes whether re-acceptance is required. Whenever a feature changes data handling (new subprocessor, new data category, new retention rule), the PR must touch this file too — that's the cheapest compliance system you can run.

---
*End of documents. Draft prepared 2026-07-17 from the Platform's actual engineering posture; all bracketed items require business/legal decisions.*
