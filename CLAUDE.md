...
|---|---|
| `docs/ENGINEERING-LOG.md` | **396 written-up fixes** — what was wrong, how it was measured, what was decided and why, and the `// GOTCHA` lines. Distilled into "Defect classes that keep recurring" below. **Grep it for a defect's shape before fixing one.** |
...
Distilled from **396 written-up fixes in `docs/ENGINEERING-LOG.md`** — the case
...
Now 503 + `Retry-After: 5` saying nothing was changed and to retry, logged at
WARN — a real fault is still a loud 500. The pool is sized in TERRAFORM
(`db_app_connection_limit`, 8 per task) and the PLAN FAILS if `api_max_count` x
it exceeds `db_app_connection_budget` — tasks x limit is what the DB sees.
...
- **A register covers the day's ROLL** (`onRollWhere`, attendance/roll.ts; `endedAt` by trigger);
  unrecorded days sit BESIDE the rate; TAKEN = `takenAt`. NEW perms need a seed re-run.
