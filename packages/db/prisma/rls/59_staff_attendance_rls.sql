-- =============================================================================
-- RLS: staff_attendance + attendance_kiosk (tenant-scoped).
--  - staff_attendance: SELECT/INSERT/UPDATE (corrections are updates), NO
--    DELETE — an attendance mark is a record.
--  - attendance_kiosk: SELECT/INSERT/UPDATE (one config row per school).
-- Row visibility further narrowed in the service (staff see own; hr sees all;
-- the kiosk TOTP secret never leaves the server).
-- Sentinel policy (docker-entrypoint idempotency key): attendance_kiosk_update
-- =============================================================================
DO $$
DECLARE t text;
BEGIN
  FOREACH t IN ARRAY ARRAY['staff_attendance','attendance_kiosk'] LOOP
