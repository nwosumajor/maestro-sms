-- =============================================================================
-- Subjects + per-class subject/teacher offerings RLS + grants
-- =============================================================================
-- Both tables are tenant-scoped read/write. Same fail-closed predicate the rest
-- of the system uses (a missing app.current_school_id GUC yields NULL and matches
-- no rows). Run as the privileged migration role. Sentinel (entrypoint
-- idempotency key): the LAST policy created, class_subject_teacher_delete.
-- =============================================================================

DO $$
DECLARE t text;
BEGIN
  FOREACH t IN ARRAY ARRAY['subject','class_subject_teacher'] LOOP
