...
export default async function CalendarPage() {
  ...
  for (const e of events ?? []) {
    entries.push({ key: `e${e.id}`, at: new Date(e.startsAt).getTime(), title: e.title, when: dateTime(e.startsAt), kind: "event", staffOnly: e.audience === "STAFF" });
  }
  for (const s of sessions ?? []) {
    for (const t of s.terms) {
      if (t.startDate) entries.push({ key: `ts${t.id}`, at: new Date(t.startDate).getTime(), title: `${t.name} begins — ${s.name}`, when: shortDate(t.startDate), kind: "term" });
      if (t.endDate) entries.push({ key: `te${t.id}`, at: new Date(t.endDate).getTime(), title: `${t.name} ends — ${s.name}`, when: shortDate(t.endDate), kind: "term" });
    }
  }
  for (const h of holidays ?? []) {
    const span = h.startDate === h.endDate ? shortDate(h.startDate) : `${shortDate(h.startDate)} – ${shortDate(h.endDate)}`;
