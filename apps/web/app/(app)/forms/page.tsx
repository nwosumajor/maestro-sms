import type { FormDto, PageDto, Serialized } from "@sms/types";
...
export default async function FormsPage() {
  ...
  const page = (await apiGet<PageDto<Serialized<FormDto>>>("/forms")) ?? { items: [], nextCursor: null };
  ...
  return (
    <AppShell schoolName={user.schoolName} userName={user.name ?? "User"} active="forms" permissions={user.permissions}>
      <div className="space-y-6">
        ...
        <FormBoard page={page} canManage={canManage} />
      </div>
    </AppShell>
  );
}
