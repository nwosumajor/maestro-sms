      ...
      <div className="space-y-6">
        ...
        <Guide
          ...
          steps={[
            ...
            // `platform.user.unlock`, which only super_admin holds. There
        ...
        {isStudent && (
          <Guide
            ...
            steps={[
              ...
              { title: "Sit CBT exams", body: mod("cbt") ? "CBT exams lists computer-based tests published for you. THE WHOLE PAPER IS ON ONE PAGE and the numbered map at the top stays with you as you scroll, so you can jump to any question, see the clock and see how many are still to do from anywhere in the paper. “Go to first unanswered” finds the next gap. Answers save as you go — a question that did not save is marked and named, so re-choose it rather than assuming — and the sitting submits itself when time ends. Scholarship qualification exams appear here too." : "When your school enables the CBT module, computer-based tests appear under CBT exams with a server-controlled clock." },
        ...
        {isTeacher && (
          <Guide
            ...
            steps={[
              ...
              { title: "Author CBT exams", body: mod("cbt") ? "CBT exams: build question banks, then schedule timed exams that sample questions per sitting. Auto-marked scores are numbers for YOUR review — you decide what they mean. A bank can be renamed or MOVED to another subject afterwards (“Rename / move” on the bank): the subject decides who may edit it and which subject its exams are marked under, so a bank filed under the wrong one is worth correcting. Its questions move with it; marks already recorded do not change." : "When your school enables the CBT module you can author question banks and timed exams under CBT exams." },
              ...
              { title: "Your register is only yours to take", body: "Attendance shows the class you are FORM TEACHER of \u2014 that register is yours and nobody else\u2019s, and the classes you merely teach a subject to are not on that page (they are on Classes, labelled with what you teach there). Everyone starts Present; mark the exceptions and save. Nothing fills a register in for you if you forget: it stays outstanding, you are reminded each afternoon, and after 7 days the correction needs a senior\u2019s approval." },
              ...
              { title: "Clock in \u2014 and out", body: "Leave \u2192 My attendance: type the code on the school\u2019s display to clock in, and type it again when you leave. Both halves matter \u2014 without a clock-out the day shows as \u201cstill in\u201d and no hours can be worked out. If nobody records anything for you, the evening close marks you absent, or on leave if an approved request covers the day." },
            ]}
          />
        )}
        ...
        {isStaffAdmin && (
          <Guide
            ...
            steps={[
              ...
              { title: "Import your students", body: "Admin → Bulk import: upload a CSV. A pupil needs only a NAME — a sign-in identifier is generated from it and your school's domain, and two pupils sharing a name both import. Upload AT MOST 200 rows at a time; a bigger file is refused before anything is created, because the sign-in slips are shown once and a request that takes too long would lose them. A DIFFERENT administrator approves the batch, and approval issues one-time passwords each pupil must change at first login. Guardians import the same way under Admin → Parents." },
              ...
              { title: "Run the desk with ID cards", body: "Scan: point a handheld scanner at a student or staff ID card and it resolves to that member \u2014 for the library desk, the gate, or an exam hall. Checking a student IN also marks them present in today\u2019s register \u2014 unless the class teacher has already marked them, which a scan never changes. It does not take the register: the class teacher still does, and marks anyone who did not scan in. A card from another school returns nothing at all."},
              ...
              { title: "A register nobody took is never filled in for them", body: "If a class teacher forgets, NOTHING marks those pupils absent \u2014 not overnight, not ever. An absence has to be recorded by somebody who looked at the room, because an unrecorded absence and a child who was present would otherwise be indistinguishable, and the school would be telling guardians their child missed a day on the strength of a form nobody filled in. What happens instead: the register stays visibly outstanding on the Attendance board with the teacher\u2019s name beside it, and a reminder goes to that teacher each afternoon in your own timezone until it is taken. After 7 days it can still be taken \u2014 it just needs a second senior to approve it." },
              ...
              { title: "Archive every term, and name it for the year it covers", body: "Admin → Long-term archives: take a permanent copy of a term or a session — enrolments, registers, subject results, invoices, approvals and the audit trail as they stood. CHOOSE THE TERM OR SESSION FROM THE LIST rather than only typing a label: the list is what BOUNDS the export, and a typed name bounds nothing. An archive labelled \u201c2025/2026\u201d that was taken with nothing selected contains every year your school has ever had, and reads as if it were one. Take one at the close of each term, and one for the session at the end of the year. Ended terms are archived automatically by a nightly sweep, so a hand-taken archive is for a session, a re-take, or a term you want early." },
              { title: "What a good archive label says", body: "Name it so somebody who was not there can tell what is inside without opening it: the school year, the term, and why it was taken \u2014 \u201c2025/2026 First Term \u2014 end of term\u201d, not \u201cbackup\u201d or today\u2019s date. The list shows the window each archive actually covers beside its label, so a mislabelled one is visible; an archive with no window shown covers EVERYTHING. Archives are permanent. Downloading one asks you to re-enter your password and returns a checksum, so whoever receives it can prove years later that the bytes were not altered." },
            ]}
          />
        )}
        ...
        {is("junior_admin") && (
          <Guide
            ...
            steps={[
              { title: "Records, and where they stop", body: "Students, classes, enrolment, guardians, timetable and documents are yours to keep current, and every change is audit-logged under your name. TAKING a register is not: it records who physically looked at the room, so it belongs to the class teacher, with the school administrator as cover. You can read attendance and refresh the rollup." },
        ...
        {(can("workflow.review.head") || can("workflow.review.hr") || can("workflow.review.principal")) && (
          <Guide
            ...
            steps={[
              { title: "Check Approvals regularly", body: "Approvals → your queue shows requests waiting at YOUR stage (leave, staff requests, purchase orders, fee-schedule runs). The staff chain is head → HR manager → principal, and each stage must be a different person." },
              { title: "Before you approve leave, look at the week", body: "A leave request names the dates and the days already used. Two more things decide it, and both are open to you: Leave → who is out shows anyone ALREADY booked off across those dates, and Timetable → cover lists the lessons the absence would leave with no teacher. Arranging the reliever is the administrators' — ask them once you have decided." },
        ...
        {can("hr.read") && (
          <Guide
            ...
            steps={[
              ...
              { title: "The staff day closes itself", body: "HR \u2192 Staff attendance: each evening, anyone with no clock-in is marked absent and anyone on approved leave is marked on leave \u2014 so the month\u2019s figures count everybody, not only the people somebody remembered to mark. \u201cClose today now\u201d does it early. A day someone clocked into and never out of is listed separately: that is the one to ask about, not an absence." },
              { title: "Correcting a staff mark", body: "You can correct anyone\u2019s mark for the last 7 days directly. Older than that it becomes an approval another senior (principal, school admin or HR manager \u2014 never you) must confirm, because a correction to an old record is a claim about the past. Nobody can mark their own attendance at all, whatever their role." },
              { title: "The gate display", body: "Running a screen at the door? Open /kiosk on it. It shows the rotating code full-screen and nothing else \u2014 no register, no names \u2014 so it is safe to leave unattended in a corridor." },
        ...
        {can("billing.read") && !isPlatform && (
          <Guide
            ...
            steps={[
              { title: "Per-seat pricing", body: "You pay per active student. Choose per-term (3 months) or per-session billing — a session is your school's whole academic year, 9 months billed because holidays are not charged, and it is 15% off against paying term by term. The session price is the same wherever your school is; the per-term price is that figure divided by however many terms your year has, so a two-semester school pays more per term and exactly the same per session. Adding students mid-period accrues a small seat top-up you can settle any time — it's added to your next renewal otherwise." },
