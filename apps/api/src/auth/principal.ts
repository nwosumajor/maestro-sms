...
export interface Principal {
  ...
  roles: string[];
  permissions: string[];
  /** Epoch ms of the password this session was issued under — see
   *  AuthService.refreshClaims, which revokes a session older than the current
   *  password. Absent on a token minted before the claim existed. */
