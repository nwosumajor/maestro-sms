...
function makeService(opts: { failKeys?: string[] } = {}) {
  ...
  const service = new PrivacyService(db as never, audit as never, storage as never, { notifyPermissionHolders: jest.fn().mockResolvedValue(0), enqueueMany: jest.fn().mockResolvedValue({ created: 0, failed: 0 }) } as never, { forSchool: async () => ({ compliance: "GDPR" }) } as never);
