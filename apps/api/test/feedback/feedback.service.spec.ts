// =============================================================================
// FeedbackService — platform feedback scoping unit tests
// =============================================================================
// Proves: a send() writes under the sender's OWN tenant tx and audits; listMine()
// reads only the sender's own rows; listAll()/review() require the privileged
// (cross-tenant) client and 503 without it; an invalid kind/status is rejected.

import { BadRequestException, ServiceUnavailableException } from "@nestjs/common";
import { FeedbackService } from "../../src/feedback/feedback.service";
import type { Principal, TenantContext, TenantTx } from "../../src/integrity/integrity.foundation";

function makeService(opts: { privilegedClient?: unknown } = {}) {
  const create = jest.fn().mockResolvedValue({ id: "fb1" });
  const findMany = jest.fn().mockResolvedValue([]);
  const tx = { platformFeedback: { create, findMany } } as unknown as TenantTx;
  ...
  return { service, audit, create, findMany };
}
...
describe("FeedbackService", () => {
  ...
  it("listAll() 503s without a privileged (cross-tenant) client", async () => {
    const { service } = makeService();
    await expect(service.listAll(principal())).rejects.toBeInstanceOf(ServiceUnavailableException);
  });

  it("review() 503s without a privileged client", async () => {
    const { service } = makeService();
