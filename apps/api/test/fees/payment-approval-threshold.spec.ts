...
describe("paymentNeedsApproval", () => {
  it("a single payment over the threshold still needs approval", () => {
    expect(paymentNeedsApproval({ kind: "PAYMENT", amountMinor: T + 1, recentPostedMinor: 0 })).toBe(true);
  });
  ...
  it("a small payment on a quiet invoice does not", () => {
    ...
    expect(paymentNeedsApproval({ kind: "PAYMENT", amountMinor: 100_000, recentPostedMinor: 0 })).toBe(false);
  });

  it("SPLITTING no longer evades it", () => {
    // The second NGN 30,000 sees the first and crosses the line.
    const half = T / 2;
    expect(paymentNeedsApproval({ kind: "PAYMENT", amountMinor: half, recentPostedMinor: 0 })).toBe(false);
    expect(paymentNeedsApproval({ kind: "PAYMENT", amountMinor: half, recentPostedMinor: half })).toBe(true);
  });

  it("three-way splitting is caught on the payment that crosses it", () => {
    const third = Math.ceil(T / 3);
    expect(paymentNeedsApproval({ kind: "PAYMENT", amountMinor: third, recentPostedMinor: 0 })).toBe(false);
    expect(paymentNeedsApproval({ kind: "PAYMENT", amountMinor: third, recentPostedMinor: third })).toBe(false);
    expect(paymentNeedsApproval({ kind: "PAYMENT", amountMinor: third, recentPostedMinor: third * 2 })).toBe(true);
  });

  it("exactly the threshold needs approval", () => {
    // >= not >: the boundary belongs on the cautious side.
    expect(paymentNeedsApproval({ kind: "PAYMENT", amountMinor: T, recentPostedMinor: 0 })).toBe(true);
    expect(paymentNeedsApproval({ kind: "PAYMENT", amountMinor: T - 1, recentPostedMinor: 0 })).toBe(false);
  });

  it("a REFUND always needs approval, however small", () => {
    // Money leaving is not judged by size.
    expect(paymentNeedsApproval({ kind: "REFUND", amountMinor: 1, recentPostedMinor: 0 })).toBe(true);
  });
});
