...
const schoolArchiveA = randomUUID();
  ...
  const cbtTheoryAnswerA = randomUUID();
  ...
  beforeAll(async () => {
    ...
    await a.query(
      `INSERT INTO teacher_unavailability (id,"schoolId","teacherId","dayOfWeek","periodId") VALUES ($1,$2,$3,'FRIDAY',$4)`,
      [teacherUnavailA, A, userA, periodA],
    );
    ...
    // Typing Race: a race + a racer in A.
    await a.query(
      `INSERT INTO typing_race (id,"schoolId","classId","hostId",difficulty,passage,"updatedAt") VALUES ($1,$2,$3,$4,'EASY','type this',now())`,
      [typingRaceA, A, classA, userA],
    );
    await a.query(
    ...
    // Chess: one game in A (board/castling JSON; content irrelevant to isolation).
    await a.query(
...
`INSERT INTO subject (id,"schoolId",name,code,"updatedAt") VALUES ($1,$2,'RLS CBT Subject','RLSCBT',now())`,
    ...
    await a.query(
      `INSERT INTO cbt_theory_answer (id,"schoolId","examId","sittingId","questionId","studentId",text,"updatedAt")
       VALUES ($1,$2,$3,$4,$5,$6,'an essay',now())`,
      [cbtTheoryAnswerA, A, cbtExamA, cbtSittingA, cbtQuestionA, userA],
    );
